...
if (import.meta.main) {
      ...
      switch (subcommand) {
        ...
        case "daemon": {
          ...
          switch (daemonSubcommand) {
            case "install": {
              console.log(`${c.bold}Installing QMD daemon...${c.reset}`);
            ...
            case "uninstall":
            case "remove": {
              console.log(`${c.bold}Uninstalling QMD daemon...${c.reset}`);
              const success = await uninstallDaemon();
              process.exit(success ? 0 : 1);
            }
            ...
            case "stop": {
              const success = await stopDaemon();
              process.exit(success ? 0 : 1);
            }
            ...
            default:
              console.error("Usage: qmd server daemon <install|uninstall|start|stop|status>");
              console.error("");
              console.error("Commands:");
              console.error("  install    Install QMD as a system service (runs on login)");
              console.error("  uninstall  Remove the system service");
              console.error("  start      Start the service");
              console.error("  stop       Stop the service");
              console.error("  status     Check service status");
              console.error("");
              console.error(`Current platform: ${getPlatformName()}`);
              process.exit(1);
          }
          break;
        }
        ...
        default:
          console.error("Usage: qmd server <start|stop|status|daemon>");
          console.error("");
          console.error("Commands:");
          console.error("  start   Start the QMD socket server (foreground)");
          console.error("  stop    Stop the running server");
          console.error("  status  Check server status");
          console.error("  daemon  Manage system service (runs on login)");
          process.exit(1);
      }
      break;
    }
  ...
  if (cli.command !== "mcp" && cli.command !== "server") {
